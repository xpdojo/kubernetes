...
- [컨테이너 스토리지 인터페이스: Container Storage Interface (CSI)](#컨테이너-스토리지-인터페이스-container-storage-interface-csi)
  - [작성자](#작성자)
  - [표기법 (Notational Conventions)](#표기법-notational-conventions)
  - [용어 (Terminology)](#용어-terminology)
  - [목적 (Objective)](#목적-objective)
    - [Goals in MVP](#goals-in-mvp)
    - [Non-Goals in MVP](#non-goals-in-mvp)
  - [솔루션 개요](#솔루션-개요)
    - [아키텍처](#아키텍처)
    - [볼륨 라이프사이클](#볼륨-라이프사이클)
  - [컨테이너 스토리지 인터페이스](#컨테이너-스토리지-인터페이스)
    - [RPC 인터페이스](#rpc-인터페이스)
      - [동시성 (Concurrency)](#동시성-concurrency)
      - [필드 요구 사항](#필드-요구-사항)
        - [사이즈 제한](#사이즈-제한)
        - [`REQUIRED` vs. `OPTIONAL`](#required-vs-optional)
      - [타임아웃](#타임아웃)
    - [에러 스키마 (Error Scheme)](#에러-스키마-error-scheme)
    - [시크릿 요구 사항](#시크릿-요구-사항)
    - [아이덴터티 서비스 RPC](#아이덴터티-서비스-rpc)
      - [`GetPluginInfo`](#getplugininfo)
        - [GetPluginInfo Errors](#getplugininfo-errors)
      - [`GetPluginCapabilities`](#getplugincapabilities)
        - [GetPluginCapabilities Errors](#getplugincapabilities-errors)
      - [`Probe`](#probe)
        - [Probe Errors](#probe-errors)
    - [컨트롤러 서비스 RPC](#컨트롤러-서비스-rpc)
      - [`CreateVolume`](#createvolume)
        - [CreateVolume Errors](#createvolume-errors)
      - [`DeleteVolume`](#deletevolume)
        - [DeleteVolume Errors](#deletevolume-errors)
      - [`ControllerPublishVolume`](#controllerpublishvolume)
        - [ControllerPublishVolume Errors](#controllerpublishvolume-errors)
      - [`ControllerUnpublishVolume`](#controllerunpublishvolume)
        - [ControllerUnpublishVolume Errors](#controllerunpublishvolume-errors)
      - [`ValidateVolumeCapabilities`](#validatevolumecapabilities)
        - [ValidateVolumeCapabilities Errors](#validatevolumecapabilities-errors)
      - [`ListVolumes`](#listvolumes)
        - [ListVolumes Errors](#listvolumes-errors)
      - [`ControllerGetVolume`](#controllergetvolume)
        - [ControllerGetVolume Errors](#controllergetvolume-errors)
      - [`GetCapacity`](#getcapacity)
        - [GetCapacity Errors](#getcapacity-errors)
      - [`ControllerGetCapabilities`](#controllergetcapabilities)
        - [ControllerGetCapabilities Errors](#controllergetcapabilities-errors)
      - [`CreateSnapshot`](#createsnapshot)
        - [The ready_to_use Parameter](#the-ready_to_use-parameter)
        - [CreateSnapshot Errors](#createsnapshot-errors)
      - [`DeleteSnapshot`](#deletesnapshot)
        - [DeleteSnapshot Errors](#deletesnapshot-errors)
      - [`ListSnapshots`](#listsnapshots)
        - [ListSnapshots Errors](#listsnapshots-errors)
      - [`ControllerExpandVolume`](#controllerexpandvolume)
        - [ControllerExpandVolume Errors](#controllerexpandvolume-errors)
      - [RPC Interactions](#rpc-interactions)
        - [`CreateVolume`, `DeleteVolume`, `ListVolumes`](#createvolume-deletevolume-listvolumes)
...
스토리지 벤더(SP)가 플러그인을 한 번 개발하면
여러 컨테이너 오케스트레이션(CO) 시스템에서 작동하도록 하는
업계 표준 "컨테이너 스토리지 인터페이스" (CSI)를 정의합니다.
...
If a `CreateVolume` operation times out, leaving the CO without an ID with which to reference a volume, and the CO _also_ decides that it no longer needs/wants the volume in question then the CO MAY choose one of the following paths:
...
It is NOT REQUIRED for a controller plugin to implement the `LIST_VOLUMES` capability if it supports the `CREATE_DELETE_VOLUME` capability: the onus is upon the CO to take into consideration the full range of plugin capabilities before deciding how to proceed in the above scena
